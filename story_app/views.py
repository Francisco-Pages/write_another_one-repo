...
class StoryDetailView(HitCountDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        user_extras = author_models.UserExtra.objects.get(
            user=self.object.author_id)
        stories_by_author = user_extras.stories.all().exclude(
            pk=self.object.pk).order_by('-published_date')[:4]
...
class ListsListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self, **kwargs):
        self.user_extra = author_models.UserExtra.objects.get(
            user=self.request.user)
...
class PinnedListsView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self, **kwargs):
        self.user_lists = author_models.UserExtra.objects.get(
...
class StorySearchResultsView(ListView):
    model = story_models.Story
    ...
    def get_context_data(self, **kwargs):
        ...
        context['user_extras'] = author_models.UserExtra.objects.select_related(
            'user')
...
class TagSearchResultsView(ListView):
